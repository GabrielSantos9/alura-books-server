...
function deletaFavoritoID(id) {
  ...
  const livrosFiltrados = favoritos.filter((favorito) => favorito.id !== id); //Filtra os livros com id diferente do id requisitado e recebe uma lista apenas com o livro deletado fora dela.
...
function insereFavorito(id) {
  ...
  const livroInserido = livros.find(livro => livro.id === id) //Find: encontra elementos dentro de uma lista. Essa constante serve para encontrar livros iguais  ao id recebido, retornará o livro que será inserido. Está buscando um livro específico na lista de livros com base no id fornecido. Se encontrado, o livro correspondente será armazenado na variável livroInserido, que pode ser usado posteriormente para adicionar aos favoritos.
  const novaListaFavoritos = [...favoritos, livroInserido] //Junção da lista de favoritos anterior junto com o novo livro inserido
   ...
   //find(): Este é um método de array que percorre todos os elementos do array e retorna o primeiro elemento que satisfaz a condição especificada na função de callback. Se nenhum elemento for encontrado, ele retorna undefined.
}
